...
| R2 | `hsi_pipeline.py` (31 erros) | **corrigido na origem (2026-09-10), mas NÃO commitado — pausado numa decisão do autor.** `apply_quality_gate_and_segment` (`hsi_pipeline.py`) e `fit_predict_pixel_plsda` (`hsi_classification.py`) ganharam `TypedDict` (`QualityGateResult`, `PixelPLSDAResult`) no lugar de `Dict[str, object]` — a raiz de fato, não um remendo por call site: os 3 `cast()` locais em `hsi_validation.py` e o `# type: ignore` em `hsi_multiway.py` ficaram desnecessários e foram removidos. `mypy` limpo nos 54 módulos do gate + `hsi_pipeline.py` (antes: 31 erros); `ruff` limpo; suíte completa 1533 passed/42 skipped (zero mudança de comportamento — só anotação de tipo, `cast()`/`TypedDict` são no-ops em runtime). **Bloqueio real**: `tests/test_contrato_api_publica.py` falha — o retorno das 2 funções, já públicas (`__all__`), mudou de assinatura textual (`Dict[str, object]` → `TypedDict`). Regravar o golden exige decidir um bump de versão correspondente (`docs/COMPATIBILITY.md`), e isso esbarra na diretriz em vigor de não mexer em versionamento antes de fechar as pendências 3/6/7 (ver memória `versionamento_v1_antes_publicar`). Diff pronto no working tree, não commitado; falta decisão do autor: (a) regravar o golden e aceitar a mudança de assinatura agora, ou (b) manter uncommitted até a versão ser destravada, ou (c) reverter para uma correção só interna (cast/TypedDict privado nos call sites, sem tocar a assinatura pública — volta ao padrão de remendo que a rodada anterior queria evitar). | |
