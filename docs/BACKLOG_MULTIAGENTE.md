...
| 12 | Nº de VLs escolhido de forma otimista | RP | **PROPAGADO (Passo 213, 2026-09-11) — autorizado explicitamente pela instrução de preparação para v1.0.** Retratação do Passo 202 confirmada e replicada (10 seeds, Passo 211: naive > aninhada em 10/10, p=0,0020) — ver nota histórica abaixo. `selecao_lv_cv_aninhada: bool = True` (novo campo `Config`/`_CONFIG_SPEC`) é o padrão a partir da v1.0: a métrica de CV reportada (`Y_cv`/`pred_lab`, e tudo que deriva dela — `balanced_accuracy`, `Q2`, ROC AUC, CV-ANOVA, bootstrap CI) passa a vir de uma CV aninhada honesta (`pipeline._selecionar_n_opt_wold`/`_construir_cv_interno`), não da mesma partição que escolheu `n_opt`. O `n_opt` do MODELO FINAL implantado não muda (continua escolhido pela CV externa sobre todo o dado — CV aninhada é avaliação, não seleção de hiperparâmetro de 1 modelo só). Configurável (`selecao_lv_cv_aninhada: false` no YAML) para quem precisa de iteração rápida — custo medido ~2,4-4,4× nesta etapa. Contra-prova de integração real (dataset privado, `level=N1`): naive=0,9154 vs. aninhada=0,9173, métrica mudou como esperado. 2 bugs reais achados e corrigidos AO IMPLEMENTAR (não presentes na medição isolada, só na integração com `executar()`): (a) fold de treino pequeno demais para sustentar CV interna própria causava split degenerado (classe vazia) → `ValueError` de LAPACK em `savgol_filter`, achado num fixture sintético de 12 amostras — corrigido com fallback honesto para o `n_opt` global quando o fold não tem >=2 grupos/amostras da classe minoritária; (b) `max_lvs` não era limitado pelo tamanho do fold interno (menor que o externo) → `PLSRegression` levantava `ValueError` de `n_components` acima do limite — corrigido limitando `max_lvs` ao menor fold em `cv_indices` (mesmo padrão de `hsi_classification.select_n_components_wold`). Suíte completa 1529+ passed/0 failed depois dos 2 fixes (ver PROGRESSO Passo 213). 4 registros novos exigidos pela mesma correção: `docs/COMPATIBILITY.md` (`apply_quality_gate_and_segment`/histórico de decisão — ver R2 acima), `docs/VALIDACAO_PUBLICA.md` §10 (nota formal de correção de v1.0), `tests/golden/contrato_api_publica.json` regravado (campo novo em `_CONFIG_SPEC`), menus da CLI/web (`cli_assistente.MENU_FIELDS`/`RISK_CLASS`/`FIELD_NAMES`/`HELP_DB`, `guaraci._menu_validation`, `app_tabs/modelo._MODELO_KEYS_VALID`) — achados por 2 testes de completude de menu que já existiam (`test_guaraci_cli.py`/`test_interfaces_configuraveis.py`), campo agora alcançável em ambas interfaces, não só a mão no YAML. |
| | | | **Histórico da retratação (Passo 202→211)**: medição original (Passo 202) foi 1 única execução (1 seed) — insuficiente para decidir, como a própria nota já registrava. `scripts/medicoes/medir_vieses_selecao_lv_replicado.py` repetiu a comparação em 10 seeds independentes + Wilcoxon pareado (mesmo padrão de `comparar_npls_pixelwise_mango.py`): naive teve balanced_accuracy maior que aninhada em 10/10 seeds (médias 0,9183 vs. 0,8756, delta médio +0,0427, p=0,0020) — direção OPOSTA à do Passo 202 (que via coincidência de 1 split tinha visto aninhada maior), e SIM na direção da hipótese original do relatório multiagente. Custo real da réplica: 2445s (~40,7 min) para as 10 seeds, reportado antes de rodar. |
