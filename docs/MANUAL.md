...
| `config_io.py` | **Fonte única da configuração**: `_CONFIG_SPEC` (campo amigável ↔ atributo), ler/gravar/validar/coagir (`carregar_config`, `salvar_config`, `_coagir_valor`, `_validar_semantico`) — extraído de `pipeline.py` |
| `resultados_io.py` | Escrita dos artefatos de uma corrida: `resumo_modelo.txt`, `model_card.md`, identificadores CSV, notas metodológicas, métricas PLS — extraído de `pipeline.py` |
| `config.py` | dataclass `Config` + fonte única de `__version__` e `_NIVEL_NOME` (módulo sem dependências) |
