...
## Passo 222 — Mutação em escopo reduzido (`predicao.py`) e reavaliação de segundo dataset para MSPC/fusão/ASCA (Agente 3, varredura final)

Instrução com 2 frentes independentes: (A) viabilizar mutation testing nos módulos em backlog por custo (`predicao.py`, `pipeline.py`, `avaliacao_modelos.py`, resto de `validacao_estatistica.py`) rodando em escopo REDUZIDO (só as funções mais críticas de UM módulo), e (B) avaliar se MSPC/fusão multibloco/ASCA+/predição multiclasse precisam de um segundo dataset público independente.

### Parte A — Mutação em escopo reduzido, `predicao.py`

`cosmic-ray` (mesma ferramenta das rodadas anteriores; `mutmut` não avaliado de novo, decisão já registrada — Windows) instalado num venv isolado (`/tmp/mutenv`, fora do repositório). Módulo escolhido: `predicao.py` — não é o mais caro (isso é `pipeline.py`), mas é o de MAIOR risco real por linha de código: é o único caminho de "aplicar o modelo já validado a uma amostra nova", onde um erro sai **em silêncio** para quem está usando o modelo em produção (o próprio docstring de `_interpolate_to_reference` já dizia isso antes desta rodada). Funções-alvo (as 3 de maior criticidade do módulo): `_interpolate_to_reference`, `predict_samples`, `quantify_sample`.

**Mecanismo de escopo reduzido**: o filtro nativo `cosmic-ray.tools.filters.line_filter` (não documentado nas rodadas anteriores desta sessão, mas já parte do pacote) marca como `SKIPPED` todo mutante fora das faixas de linha configuradas em `cosmic-ray.toml` (`[cosmic-ray.filters.line-filter]`), sem rodar teste nenhum para eles — diferente de rodar o arquivo inteiro e descartar depois. Combinado com `operators-filter` (`exclude-operators = ["core/ReplaceBinaryOperator_"]`, a explosão combinatória de trocar `+`/`-`/`*`/`/`/... por QUALQUER outro operador aritmético, já registrada como ruído cosmético nas rodadas anteriores). Resultado: dos 605 mutantes do arquivo inteiro, **195 na faixa das 3 funções**. Executado em worktree isolado (`git worktree add --detach`), nunca no diretório de trabalho principal.

**Execução parcial por orçamento de tempo desta rodada**: 20/195 mutantes rodados (5 killed, 15 survived) antes da interrupção. Dos 15 sobreviventes, **4 eram achados reais** (não cosméticos, não o ramo legado pré-2026-08-17):

1. `T_new ** 2` → `** 3` e `var_t.var(..., ddof=1)` → `ddof=0` na fórmula de Hotelling T² de `predict_samples`: nenhum teste comparava a coluna `T2` contra um oráculo numérico independente — só contra o UCL (que não é sensível à mudança em vários regimes). Fechado com `test_predizer_amostras_t2_bate_com_formula_hotelling_independente` (reimplementa a fórmula por fora, com os mesmos artefatos do pacote).
2. Guard `var_t[var_t == 0] = 1.0` → `!=`/`>`: nenhum dado sintético real produz uma componente PLS com variância de treino EXATAMENTE zero, então o guard nunca era exercitado. Fechado com `test_predizer_amostras_var_t_zero_nao_gera_divisao_por_zero`, único teste desta rodada com um `pls` fake mínimo (mesmo padrão já usado em `_PipelineFalso` para `quantify_sample`) — necessário para alcançar essa fronteira degenerada, não é mock do cálculo científico em si.
3. `np.clip(Y_soft, 0.0, 1.0)` → `np.clip(Y_soft, 0.0, 0.0)` (zera toda a predição — `classe_pred` colapsaria sempre na 1ª classe, `confiança_%` sempre 0): sobrevivia porque nenhum teste comparava `classe_pred`/`confiança_%` contra um oráculo numérico, só contra limites soltos (0–100%, "pertence ao treino") que uma predição degenerada ainda satisfaz. Fechado com `test_predizer_amostras_confianca_bate_com_softmax_normalizado_independente`.
4. `len(candidatos_ambiguos) >= 2` → `>= 1` em `quantify_sample`: fronteira exata em 1 candidato sem teste. Fechado com `test_quantify_sample_identificacao_ambigua_exige_dois_candidatos`.
5. (bônus, achado ao investigar #3) `"ad_cv_rejeitadas" in pkg and "ad_cv_n" in pkg` → `not "ad_cv_rejeitadas" in pkg and ...`: com as duas chaves presentes (caso normal, Passo 220), nenhum teste checava o CONTEÚDO das colunas `AD_ref_cv_*`. Fechado com `test_predizer_amostras_referencia_cv_da_sentinela_aparece_quando_presente`.

Cada um dos 4 (+1) testes novos foi confirmado matando o mutante específico via `cosmic-ray mutate-and-test` isolado (não só "a suíte passa depois" — o mutante foi reaplicado e o teste falhou exatamente como esperado, arquivo restaurado a cada checagem). Os outros ~11 sobreviventes das 20 execuções eram arredondamento de coluna de EXIBIÇÃO (`round(f_crit, 3)` → `,2`, `round(AD_Q, 6)` → `,5` — não usados na decisão `aceito`, que compara o valor bruto) ou o ramo LEGADO pré-2026-08-17 (`(T2_new <= t2_ucl) & (Q_new <= q_ucl)` na regra por eixo de pacotes antigos sem `pls_h0`/etc.) — deixados de fora por baixo valor/baixo uso, registrados no mapa.

**Os 175 mutantes restantes (de 195) não foram executados** — mesmo tipo de custo por suíte já documentado nas rodadas anteriores (cada mutante paga o processo `pytest` inteiro, ~15-18s mesmo na suíte mínima `test_predicao.py`). `pipeline.py`/`avaliacao_modelos.py`/resto de `validacao_estatistica.py` permanecem sem NENHUM mutante executado — não avaliados nesta rodada (o escopo pedia UM módulo). `docs/MAPA_COMPLETUDE_V1.md` atualizado com o estado exato (execução parcial, não "concluída").

Suíte afetada: `tests/test_predicao.py` (39→44 testes) + `tests/test_fluxo_cego_bloco9b.py` continuam verdes (62 passed, 2 deselected — 1 já era esperado, teste web sem `streamlit` no venv isolado). `ruff check` e `mypy` limpos em `predicao.py`.

### Parte B — Segundo dataset público: reavaliado, nenhuma ação necessária

- **MSPC**: a pergunta "precisa de um segundo dataset porque o Corn tem poucos grupos" já tinha sido feita e respondida no PRÓPRIO Passo 219/220 (mesmo dia, `docs/MAPA_COMPLETUDE_V1.md`, "Camada 3 — TEP"): dataset público confirmado (Rieth et al. 2017, Harvard Dataverse DOI 10.7910/DVN/6C3JR1, domínio público/CC0), avaliado e **conscientemente NÃO perseguido** — exigiria converter um formato de série temporal de sensores industriais (domínio bem diferente de espectroscopia) por um ganho de confiança não proporcional, já que as Camadas 1 (sintética) e 2 (Corn real) já são complementares. Nada mudou desde então (mesmo commit `HEAD`, mesma data). A ressalva "os splits sorteiam de um único conjunto de 80 amostras físicas, [IC] levemente otimista" (`docs/VALIDACAO_PUBLICA.md` §11) já está documentada honestamente, não escondida — não é motivo para reabrir a decisão do TEP sem informação nova.
- **Fusão multibloco**: já testada contra um par real complementar (Mendeley NIR8mm+MIR, mesmas amostras físicas) — resultado NEGATIVO, documentado (`docs/MAPA_COMPLETUDE_V1.md`, "Fusão multibloco — estado real"). Um segundo dataset só faria sentido para justificar fusão de nível médio/alto, que é backlog condicional, não uma lacuna de validação do nível 1 já implementado.
- **ASCA+**: o backlog registrado (Thiel et al. 2017, `ss_desbalanco` já exposto) é de COMPLETUDE de implementação (soma de quadrados Tipo III para fatores correlacionados, ~300-500 linhas estatisticamente delicadas), não de falta de dataset — o ASCA atual já foi validado com permutação por unidade experimental sobre delineamento real. Não é uma lacuna de "segundo dataset".
