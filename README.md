...
**Full documentation index:** [`docs/INDICE_PROJETO.md`](docs/INDICE_PROJETO.md)
lists every document in this repository with a one-line description.
...
- **Conformal prediction sets for classification** (LAC — Sadinle, Lei & Wasserman 2019): alongside the argmax class, batch prediction reports the *set* of plausible classes at a declared coverage level, calibrated on the external holdout — `"NOT VALIDATED"` instead of a fabricated set when the holdout lacks enough independent groups. Same split-conformal discipline already applied to regression (T1) above.
- **Portable model export** (JSON, no pickle): alongside the `.joblib`, a `<model>.portatil.json` sidecar is written for the supported preprocessing presets — no `sklearn`/`joblib` needed to apply it, and no code executes on load (eliminates the pickle-RCE risk class entirely for that file). Numerically verified against the live model (< 1e-9 difference). Scope is deliberately narrower than the full `.joblib` package: only the main prediction path (preprocessing + PLS-DA/PLS-R + classes).
- **Low-level multiblock fusion** (proof of concept): concatenates two techniques measured on the same physical samples (e.g. NIR + MIR) after preprocessing each block separately, with a guard against ever concatenating misaligned blocks. Measured honestly on a public dataset (Mendeley NIR8mm+MIR, peroxide index): low-level fusion did **not** beat the better individual block in this case — reported as a negative result, not hidden (see `docs/VALIDACAO_PUBLICA.md` §2l).
...
- **Applicability-domain drift sentinel**: accumulates the AD (in-/out-of-domain) rejection rate across successive batch-prediction runs and formally tests (one-sided exact Fisher test, two samples, H0: production rejection rate = the rate the model's own calibration rejects in cross-validation; models saved before 2026-09-19 fall back to a flagged binomial test against the nominal alpha) whether it is rising — a sign of instrument/process drift since calibration. State persists as JSON next to the model; both the CLI's and the web app's batch-prediction screens update it automatically (shared orchestration, `sentinela_deriva.hook_apos_predicao`).
...
- **Proprietary instrument format readers** (`src/guaraci/importadores_proprietarios.py`): OPUS (Bruker FT-IR/FT-NIR, `pip install guaraci-chemometrics[opus]`), SPC (Galactic/Thermo GRAMS, `[spc]`), PerkinElmer `.sp` (no extra dependency), Bruker NMR processed spectra (`pdata/<N>/1r`, `[rmn]`), and Agilent/Waters HPLC-UV/GC chromatograms (`[hplc]`) — each converts one instrument file/directory to the same `(X, Y)` array pair the rest of the pipeline consumes. Each is a standalone converter you call directly, not yet wired into the folder-based `.dx` scan above (see the module's own docstring for the scope reasoning); every reader is tested against a real instrument file or directory, never only synthetic data, except OPUS (no real `.0` file available yet — see its docstring).
...
| Technique | Status |
|---|---|
| FT-NIR, MIR, Raman, UV-Vis, GC-MS, HPLC, Generic | **Functional, validated** against real public data |
...
| IMS (GC-IMS) | **Functional, null signal** — validated against a real, already-processed public peak table (urine VOCs, colorectal cancer vs. control); result is exactly chance, honestly reported (small clinical n, not a pipeline bug) — the two larger raw-spectra datasets found earlier remain deferred by size |
...
- **Validated on public datasets, technique by technique** — see
  [`docs/VALIDACAO_PUBLICA.md`](docs/VALIDACAO_PUBLICA.md) for the current,
  complete list (it changes as techniques get closed out; this page is not
  the place to keep a count in sync). The engine is matrix-agnostic, but
  "agnostic" is an architectural property, not a validation result — a
  profile you write for a new matrix is untested until you test it.
