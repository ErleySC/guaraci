...
- **Applicability-domain drift sentinel**: accumulates the AD (in-/out-of-domain) rejection rate across successive batch-prediction runs and formally tests (one-sided exact binomial test, H0: rejection rate = nominal alpha) whether it is rising — a sign of instrument/process drift since calibration. State persists as JSON next to the model; both the CLI's and the web app's batch-prediction screens update it automatically (shared orchestration, `sentinela_deriva.hook_apos_predicao`).
